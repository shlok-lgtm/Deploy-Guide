...
| `wallet_profiles` | `main.py:301` (deleted in #206) | (`app/wallet_profile.py` → `app/indexer/profiles.py:191-210`) | **P2** | `LIVE-PATH (data-quality regression — see #228)` | Per F5 audit (PATH B halt): `main.py:301` was already deleted by #206; canonical wrapper `rebuild_all_profiles()` IS on the live path via `app/enrichment_worker.py:651` (`wallet_expansion` enrichment task). 65 attestations in 30d. The `ran (built>0)` branch went silent 2026-05-05 due to a LEFT-JOIN sampling bug in `rebuild_all_profiles(2000)` returning `built=0` consistently — tracked in #228. Wrapper also lacks `skipped_fresh` gate (2-branch shape vs canonical 3-branch). |
...
7. ~~`psi_components`~~ ✅ — two live paths (worker.py:1073-1087,
   main.py:197-210); module-canonical writer
   `app/collectors/psi_collector.py::run_psi_scoring_scheduled`. **DONE**
   in #198/#193-style fresh-base PR (supersedes #222 after intervening
   merges shifted line numbers).
